...
import { Calendar, MapPin, Clock, Search, Filter, X, ChevronLeft, ChevronRight, SlidersHorizontal } from "lucide-react";
...
function EventsContent() {
  ...
  return (
    <div className="page-container space-y-5 animate-fade-in">
      <div className="flex items-center gap-3">
        <div className="flex-1 relative">
          ...
          <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)] text-sm focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:border-transparent transition-all" />
        </div>
      ...
      {showFilters && (
        <div className="glass-card rounded-xl p-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {events.map((e: any) => (
              <Link key={e.id} href={`/events/${e.id}`} className="glass-card rounded-xl overflow-hidden card-hover group">
                {e.image_url && (
                  <div className="relative h-40 overflow-hidden">
                    <img src={e.image_url} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                ...
                {Array.from({ length: Math.min(pagination.totalPages, 5) }, (_, i) => {
                  ...
                  return (
                    <button key={pageNum} onClick={() => setPage(pageNum)}
                      className={`w-8 h-8 rounded-lg text-xs font-medium transition-all ${page === pageNum ? "bg-[var(--accent)] text-white shadow-md" : "text-[var(--text-secondary)] hover:bg-[var(--accent-subtle)]"}`}>
                      {pageNum}
                    </button>
                  );
                })}
