      ...
      <div className="space-y-8">
        <section className="glass-card rounded-2xl p-6">
          ...
          <div className="prose text-[var(--text-secondary)] space-y-3">
            <p>
              I cookie sono piccoli file di testo che i siti web salvano sul tuo dispositivo (computer, tablet, smartphone) quando li visiti. Servono a far funzionare il sito correttamente, ricordare le tue preferenze e migliorare la tua esperienza.
            </p>
            <p>
              Alcuni cookie sono <strong>necessari</strong> per il funzionamento base del sito, altri sono <strong>opzionali</strong> e servono per analisi, marketing o personalizzazione.
            </p>
          </div>
        </section>
        ...
        <section className="glass-card rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {[
              { id: "necessary", name: "Necessari", icon: "🔒", desc: "Essenziali per il funzionamento del sito", cookies: ["session_id", "csrf_token", "cookie_consent"], color: "border-green-500" },
              { id: "analytics", name: "Analitici", icon: "📊", desc: "Ci aiutano a capire come gli utenti interagiscono col sito", cookies: ["_ga", "_gid", "_gat", "_ga_*"], color: "border-blue-500" },
              { id: "marketing", name: "Marketing", icon: "📢", desc: "Per mostrare annunci pertinenti", cookies: ["_fbp", "_gcl_au", "IDE"], color: "border-purple-500" },
              { id: "preferences", name: "Preferenze", icon: "⚙️", desc: "Memorizzano le tue preferenze (lingua, tema, ecc.)", cookies: ["locale", "theme", "timezone"], color: "border-orange-500" },
            ].map((cat, i) => (
              <div key={i} className={`glass-card rounded-xl p-4 ${cat.color}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-2xl">{cat.icon}</span>
                      <h4 className="font-semibold">{cat.name} {cat.required && <span className="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2 py-0.5 rounded">Obbligatorio</span>}</h4>
                    </div>
                    <p className="text-sm text-[var(--text-muted)]">{cat.desc}</p>
                    <p className="text-xs text-[var(--text-muted)]">Cookie: {cat.cookies.join(", ")}</p>
                  </div>
                </div>
              </div>
            )}
        ...
        <section className="glass-card rounded-2xl p-6">
          ...
          <div className="prose text-[var(--text-secondary)] space-y-3">
            <p>Alcuni cookie sono impostati da servizi di terze parti che utilizziamo:</p>
            <ul className="list-disc list-inside space-y-2">
              <li><strong>Google Analytics</strong>: _ga, _gid, _gat, _ga_* (analisi traffico anonimo)</li>
              <li><strong>Google Ads / DoubleClick</strong>: IDE, _gcl_au (conversioni annunci)</li>
              <li><strong>Meta/Facebook</strong>: _fbp (pixel per annunci)</li>
              <li><strong>Vercel Analytics</strong>: Metriche performance (anonimo)</li>
            </ul>
            <p className="mt-3">Questi cookie sono gestiti direttamente dai rispettivi fornitori secondo le loro privacy policy.
          </div>
        </section>
        ...
        <section className="glass-card rounded-2xl p-6">
          ...
          <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
            <p className="text-sm"><strong>Per esercitare i tuoi diritti:</strong> Scrivi a <a href="mailto:privacy@eventinlatina.it" className="text-[var(--accent)] hover:underline font-medium">privacy@eventinlatina.it</a>. Risponderemo entro 30 giorni.</          </div>
        </section>
        ...
        <section className="glass-card rounded-2xl p-6">
          ...
          <p className="text-[var(--text-secondary)] mb-4">Utilizziamo cookie e tecnologie simili. Per dettagli completi consulta la nostra <Link href="/cookie-policy" className="text-[var(--accent)] hover:underline font-medium">Cookie Policy completa</Link>.</p>
          
          <div className="space-y-3">
            {categories.map(cat => (
              <div key={cat.id} className={`glass-card rounded-xl p-4 ${cat.required ? 'border-l-4 border-green-500' : ''}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="font-semibold">{cat.name} {cat.required && <span className="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2 py-0.5 rounded">Obbligatorio</span>}</h4>
        ...
        <section className="glass-card rounded-2xl p-6 border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20">
          ...
          <p className="text-[var(--text-secondary)]">Questa policy può essere aggiornata. La data di "Ultimo aggiornamento" in alto indica l'ultima revisione. Per modifiche sostanziali, ti informeremo via email o con banner nel sito.</        </section>
        ...
        <section className="glass-card rounded-2xl p-6">
          ...
          <div className="prose text-[var(--text-secondary)] space-y-2">
            <p>Per domande su questa Cookie Policy o sui tuoi dati:</p>
        ...
        <section className="glass-card rounded-2xl p-6 border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20">
          ...
          <p className="text-[var(--text-secondary)]">Questa Cookie Policy può essere aggiornata. La data di "Ultimo aggiornamento" in alto indica l'ultima revisione. Per modifiche sostanziali, ti informeremo via email o con banner nel sito.</        </section>
      </div>
