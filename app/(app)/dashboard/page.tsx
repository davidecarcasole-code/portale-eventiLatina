...
export default function DashboardPage() {
  ...
  return (
    <div className="page-container space-y-6 animate-fade-in">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-orange-500 via-amber-500 to-yellow-500 p-6 sm:p-8 text-white">
        ...
        <div className="relative">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold mt-2">Benvenuto su EventiNLatina</h2>
          ...
          <div className="flex gap-2 mt-4">
            <Link href="/events" className="inline-flex items-center gap-1.5 px-4 py-2 bg-white/20 backdrop-blur-sm rounded-xl text-sm font-medium text-white hover:bg-white/30 transition-all">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className={`glass-card rounded-xl p-5 stagger-${i + 1}`}>
            ...
            {categories.map((cat, i) => {
              ...
              return (
                <Link key={cat.slug} href={`/events?category=${cat.slug}`}
                  className={`flex-shrink-0 glass-card rounded-xl p-4 flex flex-col items-center gap-2.5 min-w-[100px] transition-all duration-300 stagger-${i + 1}`}>
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ backgroundColor: cat.color + "20", color: cat.color }}>
                    <Icon size={20} />
                  </div>
      ...
      {todayEvents.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">Eventi di Oggi</h3>
            <span className="text-xs text-[var(--text-muted)]">{todayEvents.length} eventi</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {todayEvents.slice(0, 6).map((e: any) => (
              <Link key={e.id} href={`/events/${e.id}`} className="glass-card rounded-xl p-4 card-hover">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm" style={{ backgroundColor: e.category_color + "20", color: e.category_color }}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <h4 className="font-medium text-sm leading-snug line-clamp-2">{e.title}</h4>
