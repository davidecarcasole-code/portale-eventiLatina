#!/usr/bin/env node
// Uso: npx tsx scripts/ingest-eventbrite.mjs [--preview]
// Importa lo scraper TypeScript e invia gli eventi all'API su Vercel

import { runEventbriteScraper } from '../lib/scraper/eventbriteScraper.ts';
...
async function sendEvents(events, token) {
  ...
  const res = await fetch(`${API_BASE}/api/scraper/ingest`, {
    ...
    body: JSON.stringify({ events, sourceName: 'Eventbrite' }),
  });
  const data = await res.json();
...
async function main() {
  ...
  console.log('=== Eventbrite Ingest Tool ===');

  const events = await runEventbriteScraper();
  console.log(`[Eventbrite] ${events.length} eventi trovati`);

  if (events.length === 0) {
    console.log('[Eventbrite] Nessun evento trovato.');
    return;
  }
