...
function resolveEventCategory(e: ScrapedEvent, catMap: Map<string, number>): number | null {
  if (isKidsEvent(e.title, e.description)) {
    const kidsId = catMap.get('bambini');
    if (kidsId) return kidsId;
  }
  return resolveCategoryId(e.category_id, catMap);
}
...
export async function backfillKidsCategories(): Promise<number> {
  const prisma = await getPrisma();
  try {
    const kidsCat = await prisma.category.findUnique({ where: { slug: 'bambini' } });
    if (!kidsCat) {
      console.log('[Backfill] No "bambini" category in DB — skipping');
      return 0;
    }
    ...
    for (const ev of candidates) {
      if (isKidsEvent(ev.title, ev.description || undefined)) {
        await prisma.event.update({ where: { id: ev.id }, data: { categoryId: kidsCat.id } });
        updated++;
      }
    }
    console.log(`[Backfill] Re-categorized ${updated} events as bambini`);
    return updated;
  } catch (err: any) {
    console.error(`[Backfill] Failed: ${err.message?.slice(0, 100)}`);
    return 0;
  }
}
