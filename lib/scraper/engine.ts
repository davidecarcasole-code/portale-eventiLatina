...
export async function runScraper(sourceType?: string): Promise<ScraperResult[]> {
  ...
  for (const src of sources) {
    if (src.type === 'facebook') {
      if (!src.facebookPageId || !src.facebookAccessToken) {
        console.log(`[Scraper] Facebook source "${src.name}" missing page ID or access token — skipping`);
        continue;
      }
      console.log(`[Scraper] Source: ${src.name} (Facebook)...`);
      const { scrapeAllFacebookPages } = await import('./facebookScraper');
      const { events, results } = await scrapeAllFacebookPages([{
        pageId: src.facebookPageId,
        accessToken: src.facebookAccessToken,
        name: src.name,
      }]);
      console.log(`[Scraper] ${src.name}: ${events.length} events scraped`);
      ...
      console.log(`[Scraper] ${src.name}: ${inserted} new events inserted`);
      results.push({ source: src.name, found: events.length, inserted });
      try {
