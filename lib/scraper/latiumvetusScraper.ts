...
const BASE = 'https://www.latiumvetus.it';
const LISTING = `${BASE}/visite/`;
...
function detectCategory(text: string): string {
  ...
  if (/escursione|trekking|camminata|sentiero|visita guidata/.test(lower)) return 'escursioni';
  if (/cultura|mostra|museo|arte|conferenza|storia|archeolog/.test(lower)) return 'cultura';
...
function isLatinaProvince(text: string): boolean {
  const lower = text.toLowerCase();
  return /latina|circeo|sabaudia|terraccina|pontino|anzio|nettuno|aprilia|cisterna|sermoneta|sonnino|maenza|agro pontino| LT /i.test(lower);
}

export async function runLatiumVetusScraper(): Promise<ScrapedEvent[]> {
  console.log('[LatiumVetus] Starting...');
  ...
  for (let page = 1; page <= 5; page++) {
    ...
    try {
      ...
      const events = $('article, .visit, [class*="visit"], [class*="event"], .entry, .post');
      if (events.length === 0) {
        console.log(`[LatiumVetus] Page ${page}: no more events`);
        break;
      }
      ...
      events.each((_, el) => {
        ...
        all.push({
          ...
          source_name: 'Latium Vetus',
        });
      });

      console.log(`[LatiumVetus] Page ${page}: ${events.length} events, ${newCount} new`);
    } catch (err: any) {
      console.error(`[LatiumVetus] Page ${page} error: ${err.message?.slice(0, 100)}`);
      break;
    }
  }

  console.log(`[LatiumVetus] Total: ${all.length} unique events`);
  return all;
}
